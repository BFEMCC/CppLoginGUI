...
/*说明：
2023年4月19日0时
void PointerInIt(int w, int h, COLORREF bkcolor = WHITE)
函数由我手动添加，解决了创建画布指针无法初始化问题。
联系：Bilbili：EMC方 https://b23.tv/24iWMNT
由于原代码作者许多函数未添加功能注释，且帮助文档中没有相应说明，迫于时间效率等原因，没有仔细阅读查找源码中是否已有该功能函数，
为追求方便，直接在头文件中定义函数，见谅。
而是手动添加了该函数，本人使用时未出现bug异常，但未经全面测试，请谨慎使用
在此感谢HiEasyX作者

***2023年5月6日21时 将函数名由PointerInIt改为InIt
*/
/*****************************************************************************************************/
		void InIt(int w, int h, COLORREF bkcolor = WHITE)
		{
			CleanUpSettings();
			Resize(w, h);
			SetBkColor(bkcolor);
